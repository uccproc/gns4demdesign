...
topic = 'case5_mp6_r005_'#yujiang, 22.01.15, added
...
# model_path = r'E:\OneDrive - University College Cork\recap_2310_12\GNS_for_2D_Dambreak_frictionrestitution\models\17_granular_2D_50_friction_restitution_DoE_2nd\case5_mp6_r005_0\case5_mp6_r005_110000checkpoint_evalbest.pth.tar' #yujiang, if not None, then it goes to Training mode(resume) or Evaluation mode
...
#        print('particle_type_embeddings is', particle_type_embeddings)
        ...
        connectivity = torch.tensor(self._connectivity_radius)
        #edge_features.append(overlap_distances/connectivity.to(self._device)) #yujiang, comment this if not needed
...
#        print('current',current_positions)
    ...
    def predict_accelerations(self, next_position, position_sequence_noise, position_sequence, n_particles_per_example, particle_types, friction, restitution):
...
def train(simulator):
    ...
    #print(start_time_1 - start_time_0, "period1_")
    i = 0
...
#        optimizer = torch.optim.Adam(simulator.parameters(), lr=lr_init)
        ...
        best_loss_momentum = torch.load(model_path)['best_loss_momentum']
    ...
    #print(start_time_2 - start_time_1, "period2_")
    start_time_6 = start_time_2
    ...
    try:
        for features, labels in ds:
            ...
            sampled_noise = get_random_walk_noise_for_position_sequence(features['position'], noise_std_last_step=noise_std)
            features['position'] = torch.tensor(features['position']).to(device)
            start_time_3 = time.clock()
            #print(start_time_3 - start_time_6, "period3_")

            non_kinematic_mask = (features['particle_type'] != kinematic_particle_id).clone().detach().to(device) # yujiang, previously '!= 3'
            sampled_noise = sampled_noise.clone().detach().to(device)
            sampled_noise *= non_kinematic_mask.view(-1, 1, 1)
#            print(sampled_noise, 'sampled_noise')
#            print(labels, 'labels')
            ...
            #print(start_time_5 - start_time_3, "period5_")

            #pred = torch.tensor(pred).to(device)
            #target = torch.tensor(target).to(device)
#            pred = pred.clone().detach().requires_grad_(True).to(device)
#            target = target.clone().detach().requires_grad_(True).to(device)
            ...
            loss2 = (pred.sum(dim=0) - target.sum(dim=0)) ** 2
            ...
            start_time_6 = time.clock()
            #print(start_time_6 - start_time_5, "period6_")
            ...
            best_loss_momentum = min(loss + 1e-3 * loss2.sum(dim=-1) / pred.shape[0], best_loss_momentum)
            ...
            if step % eval_steps == 0: #yujiang, 22.01.11, this block is uncommented
                ...
                start_time_7 = time.clock()
                #print(start_time_7 - start_time_6, "period7_")
            ...
            if step % save_steps == 0: #yujiang, changed from simulator.save(LOG_DIR+'model.pth')
                simulator.save({
                    ...
                    'best_loss_momentum': best_loss_momentum,
                    'lr': lr_new,
...
def infer(simulator):
    ...
    eval_rollout(ds, simulator, num_steps=num_steps, num_eval_steps=25, save_results=True, device=device) #yujiang, num_eval_steps=5 is added
    print("Finished Training ! ", "Duration time:", datetime.now()-start2)
...
if __name__ == '__main__':
    ...
    simulator = Simulator(
        ...
        node_in=19, #yujiang, originally 30 = 10+4+16, 2023.3.31: add 1 from 13 because friction is added
        ...
        latent_dim=128,
        num_message_passing_steps=6, #yujiang, originally 10
        mlp_num_layers=2,
        mlp_hidden_dim=128,
